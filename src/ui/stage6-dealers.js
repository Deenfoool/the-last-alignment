...
(function () {
  ...
  function readEnvelope() {
    try { const raw = safeStorage() && safeStorage().getItem(SAVE_KEY); return raw ? JSON.parse(raw) : null; } catch (error) { return null; }
  }
  function safeMode() {
    try { return Content && Content.load().mode === Content.MODES.SAFE; } catch (error) { return false; }
  }
  function activeDealerId() {
    const envelope = readEnvelope();
    const state = envelope && envelope.state;
    return Dealers.normalizeId(state && (state.dealerId || state.rules && state.rules.dealerId) || selectedId);
  }
  ...
  function injectSelector() {
    ...
    $$('input[name="dealerId"]', fieldset).forEach((input) => input.addEventListener("change", () => {
      selectedId = Dealers.saveSelection(input.value, safeStorage());
      syncSelector();
    }));
    syncSelector();
  }
  ...
  function syncSelector() {
    ...
    if (preview) preview.innerHTML = `<b>${escapeHtml(profile.name)}</b> · ${escapeHtml(profile.archetype)} · ${profile.maxHp} здоровья · ${profile.maxEnergy} энергии<br>${escapeHtml(profile.ability.name)}: ${escapeHtml(profile.ability.description)}`;
    const setupText = $(".setup-heading p");
    if (setupText && !$("#setupOverlay").hidden) setupText.textContent = `Выбери противника, время хода и цену промедления. Сейчас за столом: ${profile.name}.`;
  }
  ...
  function injectSceneElements() {
    ...
    if (!$("#dealerPortrait")) {
      ...
      portrait.id = "dealerPortrait";
      portrait.className = "dealer-portrait";
      portrait.setAttribute("aria-hidden", "true");
      scene.insertBefore(portrait, $("#dealerHand"));
    }
    if (!$("#dealerAbilityPlate")) {
      const plate = document.createElement("section");
      plate.id = "dealerAbilityPlate";
      plate.className = "dealer-ability-plate";
      plate.innerHTML = '<span>СПОСОБНОСТЬ</span><strong></strong><p></p>';
      scene.append(plate);
    }
  }
  ...
  function applyProfile() {
    ...
    if (portrait) portrait.style.backgroundImage = `url("${Dealers.portraitDataUri(profile)}")`;
    const name = $(".dealer-nameplate strong");
    if (name) name.textContent = profile.name.toUpperCase();
    const quote = $("#dealerQuote");
    if (quote) {
      const envelope = readEnvelope();
      const dealer = envelope && envelope.state && envelope.state.actors && envelope.state.actors.dealer;
      const key = dealer && dealer.hp / dealer.maxHp <= .35 ? "low" : "intro";
      quote.textContent = Dealers.quoteFor(profile, key, safe);
    }
    const ability = $("#dealerAbilityPlate");
    if (ability) {
      $("strong", ability).textContent = profile.ability.name;
      $("p", ability).textContent = profile.ability.description;
      ability.dataset.tier = profile.tier;
    }
    const dealerHudTitle = $("#dealerHud h2");
    if (dealerHudTitle) dealerHudTitle.textContent = `ДИЛЕР · ${tierNames[profile.tier]}`;
    const catalogBadge = $("#catalogBadge");
    if (catalogBadge) catalogBadge.textContent = `${profile.name.toUpperCase()} · ИИ ${profile.difficulty}`;
    if (lastDealerId !== dealerId) {
      lastDealerId = dealerId;
      document.body.classList.remove("dealer-arrival");
      void document.body.offsetWidth;
      document.body.classList.add("dealer-arrival");
    }
  ...
  function showAbilityEvents(profile, safe) {
    ...
    if (!float) return;
    float.textContent = event.type === "DEALER_PHASE_CHANGED" ? (safe ? "ВТОРАЯ ФАЗА" : "ПРАВИЛА ИЗМЕНИЛИСЬ") : profile.ability.name.toUpperCase();
  ...
  function scheduleApply() {
    if (scheduled) return;
    scheduled = true;
    requestAnimationFrame(() => { scheduled = false; applyProfile(); });
  }
