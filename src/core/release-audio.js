...
(function (root, factory) {
  ...
  function apply(settings) { const value = normalize(settings || load(root && root.localStorage)); if (ensure()) { master.gain.setTargetAtTime(value.enabled ? value.master : 0, context.currentTime, .02); musicGain.gain.setTargetAtTime(value.music ? value.musicVolume : 0, context.currentTime, .05); } if (!value.music) stopMusic(); else if (unlocked) startMusic(); return value; }
  function unlock() { const audio = ensure(); if (!audio) return false; unlocked = true; if (audio.state === "suspended") audio.resume().catch(() => null); apply(load(root.localStorage)); return true; }
  function tone(frequency, duration, type, volume, slide) {
    const settings = load(root && root.localStorage); if (!settings.enabled || !unlock()) return;
  ...
  function startMusic() {
    const settings = load(root && root.localStorage); if (!settings.enabled || !settings.music || !unlock() || musicNodes.length) return;
